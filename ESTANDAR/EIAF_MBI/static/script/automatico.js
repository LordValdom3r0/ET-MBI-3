...
function loadEvents(selector1, selector2, selector3) {
    ...
    fetch(dominio + '/api/get/eventos')
        ...
        .then(function (data) {
            ...
            $('[data-tooltip="tooltip"]').css({
                "cursor": "pointer"
            }).tooltip(); // Muestra un tooltip con información cuando se coloca el mouse sobre un elemento con este atributo
        })
...
function update_modularities() {
    ...
    fetch(dominio + '/update/modularities', {
        ...
        .then(result => {
            console.log('Resultado:', result);
            var ilx_failure = document.getElementById("ilx_fail_alert");
            ilx_failure.innerHTML = '';
            ...
            if ((Object.keys(result[flujo])).length == 0) {
                ...
                (Object.keys(result[flujo])).forEach(function (valor, indice, array) {
                    ...
                    ilx_failure.innerHTML += '<p>'+ indice + '.- ' + valor + ' --> ' + result[flujo][valor] + '</p>' + '<hr>';
                });
...
function searchDAT() {
    ...
    showedArray.forEach(evento => {
        ...
        if (busqueDat.length !== 0) {
            fetch(dominio + "/api/get/" + evento + "/modularidades/ID/>/0/_/=/_")
                .then(data => data.json())
                .then(data => {
                    var arrayPedidos = data.MODULARIDAD
                    console.log(data);
            ...
            document.getElementById(evento).classList.add('gray-light')
        }
    });
}