        ...
        .then(function (data) {
            ...
            myDats.forEach(pedido => {
                ...
                for (let i = 0; i < keys.length; i++) {
                    const evento = keys[i]
                    fetch(`${dominio}/api/get/${evento}/modularidades/MODULARIDAD/=/${pedido}/_/=/_`)
                        .then(data => data.json())
                        .then(data => {
                            console.log(data);
                            if (document.getElementById(data.MODULARIDAD) && data.MODULARIDAD != undefined) {
                                var encontrado = document.getElementById(`${data.MODULARIDAD}`)
                                
                                if (encontrado.innerHTML.length <= 18) {
                                    encontrado.classList = 'encontrado'
                                    encontrado.innerHTML = `${data.MODULARIDAD} - ${evento}`;
                                
                                }else{
                                    var repetido = document.createElement('p');
                                    repetido.classList = 'encontradoXn'
                                    repetido.innerHTML = `${data.MODULARIDAD} - ${evento}`;
