...
nameDate = estacion.includes("PUR_MBM")|| estacion.includes("CONVEYOR_MBM_1")? "DATETIME": nameDate;
nameActive = estacion.includes("PUR_MBM")|| estacion.includes("CONVEYOR_MBM_1")? "ACTIVE": nameActive;
function cleardiv(){
...
function cargar_produccion(){
  ...
  .then(data=>{
    ...
    colnames.splice(colnames.indexOf(nameActive),1,"ID",nameActive);
...
function guardar_edit(){
  ...
  if (usuarioadd.length===0 || passadd.length===0) {
    ...
    if (flagusuario == true & flagpass == true) {
      ...
      .then(function (data){
        console.log(data);
        location.href = "edit_usuarios.php";
      })
      .catch(function(err) {
        console.log(err);
      });
