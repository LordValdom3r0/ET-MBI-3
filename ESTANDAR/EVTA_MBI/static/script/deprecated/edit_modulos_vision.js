...
// console.log("ARRAY PDCS",pdcs_array);
  ...
  fetch(dominio + "/api/get/" + DBEVENT + "/modulos_fusibles/ID/=/" + edit_id + "/_/=/_")
    ...
    .then(data => {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        if (keys[i] == 'ID' || keys[i] == 'MODULO') {
          ...
          if (data[keys[i]] == '{}') {
            ...
            var fusibles = Object.keys(JSON.parse(data[keys[i]])[caja_name])
            // console.log(fusibles);

           //console.log('EL BOX ARRAY',box_array);
          
           if (caja_name[0].includes('PDC-S')) {
              
            //console.log(JSON.parse(data[keys[i]])[caja_name]);
            var verificar_PDCS = [changePDCS.getMayusCaja()]; 
            fuses_value[verificar_PDCS] = JSON.parse(data[keys[i]])[caja_name]
            //console.log(fuses_value[verificar_PDCS]);
            
           }            
           find_f96 = fusibles.filter(item => item.includes('F96'))
           switch (true) {

              case  find_f96.length > 0:
                
                var minuscula = `${find_f96[0]}`.toLowerCase();
                console.log(find_f96[0]);
                changeF96.cambiarVariante(`${find_f96[0]},${minuscula}`)
                var verificar_F96 = [changeF96.getMayusVariante()]; 
                //fUSES_VALUE = { 'F96': 'F96': "VACIO" }
                // El primer 'F96' cambiara respecto a la variante
                console.log(JSON.parse(data[keys[i]])[caja_name]);
                console.log(JSON.parse(data[keys[i]])[caja_name][verificar_F96]);
                fuses_value[changeF96.getMayusCaja()]['F96'] = JSON.parse(data[keys[i]])[caja_name][verificar_F96]
                console.log(fuses_value['F96']);    
                break;
            ...
            for (var j = 0; j < fusibles.length; j++) {
              ...
              // console.log("Valor del Fusible: ",JSON.parse(data[keys[i]])[caja_name][fusibles[j]]);
              ///Verficando caja Variante 
              // var verificar_PDCS = caja_name[0].includes(changePDCS.getMayusCaja())? [changePDCS.getMayusCaja()] : caja_name;
              ...
              // fuses_value[verificar_PDCS][fusibles[j]] = JSON.parse(data[keys[i]])[caja_name][fusibles[j]]
              cargar_recuadros();

              // if (JSON.parse(data[keys[i]])[caja_name][fusibles[j]] == 1) {
              // 	switch (caja_name){
              // 		case"PDC-S":
              // 		pdcs_array.push(fusibles[j]);
              // 		console.log("PDCS ARRAY NUEVO: ",pdcs_array);
              // 		break;
              // 	}
              // }
            }
            // console.log(JSON.parse(data[keys[i]])[caja_name]['S2-1'])
          }
        }
      }
      ...
      function cargar_recuadros() {
        ...
        if (caja_name == "PDC-R" && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] != "vacio") {
          // console.log("AQUI ESTÁ LA CAJA PDC-R");
          pdcr_array.push(fusibles[j]);
          // console.log("PDC-R ARRAY NUEVO: ",pdcr_array);
          $("#pdcr_option > option[value='PDC-R']").attr("selected", true);
          pdcr_caja = "pdcr"
          pdcr_caja_to_db = "PDC-R"
          document.getElementById('caja_pdcr').style.display = "block"
          cargar_imagen_pdcr();
        }
        if (caja_name == "PDC-RMID" && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] != "vacio") {
          // console.log("AQUI ESTÁ LA CAJA PDC-RMID");
          if (!fusibles[j].includes("F96")) {
            pdcr_1_array.push(fusibles[j]);
          } 
          if (fusibles[j].includes("F96")) {
            box_array[changeF96.getMayusVariante()] = [changeF96.getMayusCaja()];
            //console.log(box_array[changeF96.getMayusVariante()]);

            cargar_imagen(changeF96);
            
          }
          
          // console.log("PDC-RMID ARRAY NUEVO: ",pdcr_1_array);
          $("#pdcr_option > option[value='PDC-RMID']").attr("selected", true);
          pdcr_caja = "pdcr_1"
          pdcr_caja_to_db = "PDC-RMID"
          document.getElementById('caja_pdcr_1').style.display = "block"
          cargar_imagen_pdcr_1();
        }
        if (caja_name == "PDC-RS" && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] != "vacio") {
          // console.log("AQUI ESTÁ LA CAJA PDC-RMID");
          pdcr_small_array.push(fusibles[j]);
          // console.log("PDC-RMID ARRAY NUEVO: ",pdcr_small_array);
          $("#pdcr_option > option[value='PDC-RS']").attr("selected", true);
        ...
        if (caja_name == "PDC-E" && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] != "vacio") {
          ...
          cargar_imagen_pdce(data);
        }
        if (caja_name == "PDC-E_AMG" && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] != "vacio") {
          pdce_amg_array.push(fusibles[j]);
          $("#pdce_amg_option > option[value='PDC-E_AMG']").attr("selected", true);
          pdce_caja = "pdce_amg"
          pdce_caja_to_db = "PDC-E_AMG"
          document.getElementById('caja_pdce_amg').style.display = "block"
          cargar_imagen_pdce_amg(data);
        }
        if (caja_name[0].includes("F96") && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] != "vacio") {
          // console.log("AQUI ESTÁ LA CAJA F96");
          //box_array[caja_name[0]] = fusibles[j];
          // console.log("F96 ARRAY NUEVO: ",f96_array);
          cargar_imagen(changeF96);
        }
        
        if (caja_name[0].includes("PDC-S")  && JSON.parse(data[keys[i]])[caja_name][fusibles[j]] !== "vacio") {
          console.log("AQUI ESTÁ LA CAJA PDC-S");
          var minuscula = `${caja_name[0].replace('-','')}`.toLowerCase();
          console.log(caja_name[0],"CAJA: ",box_array[caja_name[0]]);
          changePDCS.cambiarVariante(`${caja_name[0]},${minuscula}`)
          if (!box_array[caja_name[0]]) {
            box_array[caja_name[0]] = []
          }
          console.log(fusibles);
          ...
          if (pixelx >= fuses_BB['TBLU'][keys_tblu[i]][0][0] && pixelx <= fuses_BB['TBLU'][keys_tblu[i]][1][0] && pixely >= fuses_BB['TBLU'][keys_tblu[i]][0][1] && pixely <= fuses_BB['TBLU'][keys_tblu[i]][1][1] && color != "vacio" && caja == "tblu") {
            ...
            switch (color) {
              case "cafe":
                ...
                break;
              case 'rojo':
                if (fusible === 'F464' || fusible === 'F463' || fusible === 'F462' ||
                  fusible === 'F449' || fusible === 'F448' || fusible === 'F447' || fusible === 'F420' ||
                  fusible === 'F419' || fusible === 'F418') {
                  amperaje = '50';
                } else {
                  amperaje = '10';
                }
...
function cargar_imagen_pdce(cajasData) {
...
function cargar_imagen_pdce_amg(data) {
...
function cargar_imagen_pdcs1(data) {
      ...
      function preview() {
        (async function () {
          ...
          for (let i = 0; i < pdcs1_array.length; i++) {
            ...
            console.log(data);
            let box = JSON.parse(data["CAJA_2"]);
            console.log(box);
            let fusibleColocado = box["PDC-S1"][pdcs1_array[i]];
            color = fusibleColocado;
            let cavidadx =
            ...
            orientacion = yDistance > xDistance ? "v" : "h";
            // console.log(`Fusible colocado: ${fusibleColocado} Cavidad: ${cavidad}
...
function change_caja_pdce() {
  ...
  if (document.getElementById("pdce_option").value === "PDC-E") {
      ...
      if (hold_config) {
        ...
        for (let index = 0; index < pdce_amg_array.length; index++) {
          ...
          if (fuses_value["PDC-E_AMG"].hasOwnProperty(pdce_amg_array[index])) {
            ...
            //console.log("fusible en pdce:", fuses_value["PDC-E_AMG"][pdce_amg_array[index]]);
          } else {
    ...
    //var t1 = new ToolTip_pdce(img_pdce, "This is a tool-tip", 150);
    cargar_imagen_pdce();
    // console.log(fuses_value["PDC-E"])
    // console.log(fuses_value["PDC-E_AMG"])
...
function ToolTip_pdcr_1(img_pdcr_1, text, width) {
  //console.log(document.getElementById('toolTip_pdcr_1'));
  var me = this, // self-reference for event handlers
    div = document.getElementById('toolTip_pdcr_1') ? document.getElementById('toolTip_pdcr_1') : document.createElement("div"), // the tool-tip div
    parent = img_pdcr_1.parentNode, // parent node for img_pdcr_1
    visible = false;
  //console.log(div);
  div.id = 'toolTip_pdcr_1'
  parent.appendChild(div) // current status
  // show the tool-tip
  this.show = function (pos) {
    if (!visible) { // ignore if already shown (or reset time)
      visible = true; // lock so it's only shown once
  ...
  function check(e) {
    ...
    for (i = 0; i < keys_cavidad.length; i++) {
      if (!visible && pos.x >= fuses_BB["PDC-RMID"][keys_cavidad[i]][0][0] && pos.x <= fuses_BB["PDC-RMID"][keys_cavidad[i]][1][0] && pos.y >= fuses_BB["PDC-RMID"][keys_cavidad[i]][0][1] && pos.y <= fuses_BB["PDC-RMID"][keys_cavidad[i]][1][1]) {
        ...
        let unidad = amperaje === 'N/A' ? 'N/A' : amperaje + ' Amps';
        div.innerHTML = `Cavidad: ${cavidad}<br>Fusible: ${fusible_tooltip}<br>Amperaje: ${unidad}`;
        //console.log(`Cavidad: ${cavidad}<br>Fusible: ${fusible_tooltip}<br>Amperaje: ${unidad}`);
        me.show(posAbs); // show tool-tip at this pos
      } else setDivPos(posAbs);
    } // otherwise, update position
  }
...
function ToolTip_pdcr_small(img_pdcr_small, text, width) {
  var me = this, // self-reference for event handlers
    // the tool-tip div
    div = document.getElementById('toolTip_pdcr_small') ? document.getElementById('toolTip_pdcr_small') : document.createElement("div"), // the tool-tip div
    parent = img_pdcr_small.parentNode, // parent node for img_pdcr_small
    visible = false;
  div.id = 'toolTip_pdcr_small'
  // show the tool-tip
...
//   var me = this, // self-reference for event handlers
//     //div = document.createElement("div"),      // the tool-tip div
//     div = document.getElementById('toolTip_f96') ? document.getElementById('toolTip_f96') : document.createElement("div"), // the tool-tip div
...
//         if (fuses_value["PDC-RMID"]['F96'] == "vacio" || fuses_value["PDC-RMID"]['F96'] == undefined) {
//           fusible_tooltip = "Vacío";
//         } else {
...
function ToolTip_tblu(img_tblu, text, width) {
  var me = this, // self-reference for event handlers
    div = document.getElementById('toolTip_tblu') ? document.getElementById('toolTip_tblu') : document.createElement("div"), // the tool-tip div
    // div = document.createElement("div"),      // the tool-tip div
...
function ToolTip_pdcd(img_pdcd, text, width) {
  var me = this, // self-reference for event handlers
    div = document.getElementById('toolTip_pdcd') ? document.getElementById('toolTip_pdcd') : document.createElement("div"), // the tool-tip div
...
function ToolTip_pdcp(img_pdcp, text, width) {
  var me = this, // self-reference for event handlers
    div = document.getElementById('toolTip_pdcp') ? document.getElementById('toolTip_pdcp') : document.createElement("div"), // the tool-tip div
...
function ToolTip_pdce(img_pdce, text, width) {
  var me = this, // self-reference for event handlers
    div = document.getElementById('toolTip_pdce') ? document.getElementById('toolTip_pdce') : document.createElement("div") // the tool-tip div
...
function ToolTip_pdce_amg(img_pdce_amg, text, width) {
  var me = this, // self-reference for event handlers
    div = document.getElementById('toolTip_pdce_amg') ? document.getElementById('toolTip_pdce_amg') : document.createElement("div"), // the tool-tip div
    parent = img_pdce_amg.parentNode, // parent node for img_pdce_amg
    visible = false; // current status
  div.id = 'toolTip_pdce_amg';
  // show the tool-tip
  this.show = function (pos) {
    if (!visible) {
      // ignore if already shown (or reset time)
  ...
  if (modulo_db.length == 0) {
    ...
    if (allEmpty_pdcs) {
      console.log("PDCS vacio, volviendo al original");
        changePDCS.agregarVariante('PDC-S,pdcs')
      }
    if (allEmpty_f96){
      console.log("F96 vacio, volviendo al original");
      changeF96.agregarVariante('F96,f96')
    }
