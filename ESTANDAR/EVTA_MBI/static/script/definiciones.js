...
console.log(document.getElementById("caja_mb"));
...
function variantesPDC_edit(){
    ...
    .then(data=>{
      ...
      colnames.unshift("ID");
    //   colnames.splice(colnames.indexOf(nameDate),1);
    //   colnames.splice(colnames.indexOf("USUARIO"),1);
    //   colnames.splice(colnames.indexOf("VARIANTE"),1,"VARIANTE","USUARIO", nameDate);

      console.log("COLUMNAS: ", colnames);
      var filas = Array.isArray(data[colnames[0]]) ? data[colnames[0]].length : 1;
      ...
      for (i = 0; i < filas; i++) {
          ...
          for (j = 0; j < colnames.length; j++) {
            ...
            var elemento = Array.isArray(data[colnames[j]])? data[colnames[j]][i] :  data[colnames[j]];
          ...
          botondos.value = Array.isArray(data["MODULO"])? data["MODULO"][i]: data["MODULO"];
